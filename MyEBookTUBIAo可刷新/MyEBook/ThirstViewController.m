...
@interface ThirstViewController ()<UITableViewDelegate,UITableViewDataSource,NSURLSessionDataDelegate>{
...
@property(nonatomic,strong)NSString * strep_time;
@property(nonatomic,strong)NSString * strep_id;
@end

@implementation ThirstViewController
#pragma mark 轮播图数据的加载  self.mArrayOfLuBoTu
-(void)loadDataOfLuBoTu{
    NSString * str=@"http://lib.wap.zol.com.cn/ipj/docList/?v=11.0&class_id=210&page=1&vs=and501&retina=1&modename=M2NOTE";
    //1.生成URL
...
-(NSMutableArray *)loadData:(NSString *)fileName{
    ...
    NSURLSessionDataTask * task=[session dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        ...
        if(data){
            ...
            self.strep_time=dict[@"ep_time"];
            self.strep_id=dict[@"ep_id"];
            //
    ...
    _header =  [XDRefreshHeader headerOfScrollView:self.tableView refreshingBlock:^{
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString *fileName = [NSString stringWithFormat:@"http://lib.wap.zol.com.cn/ipj/docList/?v=11.0&class_id=210&page=1&vs=and501&retina=1&modename=M2NOTE"];
    ...
    _footer = [XDRefreshFooter footerOfScrollView:self.tableView refreshingBlock:^{
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSString *firstName = [NSString stringWithFormat:@"http://lib.wap.zol.com.cn/ipj/docList/?v=11.0&class_id=210&page=%ld&vs=and501&retina=1&modename=M2NOTE",count];
                NSString * fileName=[firstName stringByAppendingFormat:@"&ep_id=%@&ep_time=%@",self.strep_id,self.strep_time];
                
                [weakSelf loadData:fileName];
